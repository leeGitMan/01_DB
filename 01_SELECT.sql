 ...
 * 
 * - 데이터를 조회(SELECT)하면 조건에 맞는 행들이 조회가 된다.
...
-- <컬럼 값 산술 연산>
-- 컬럼값 : 테이블 내 한 칸에 작성된 값(DATA)

-- 문제 : EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉(급여 * 12) 조회
SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 FROM EMPLOYEE;
FROM EMPLOYEE;

-- DB에서는 문자+숫자 출력 시 에러가 난다.

SELECT EMP_NAME + 10 FROM EMPLOYEE;
-- SQL Error ORA-01722: 수치가 부적합 합니다
...
-- SYSDATE : 시스템 상 현재 시간(날짜)를 나타내는 상수
-- (게시판 등록 시간 저장)


SELECT SYSDATE FROM DUAL;
-- 현재 시간만 조회하기
-- DUAL(DUmmy tAbLe) : 테이블 : 가짜테이블(임시조회용 테이블)


SELECT SYSDATE -1, SYSDATE, SYSDATE +1
FROM DUAL;
-- 날짜 + 산술연산(+,-)
-- 오늘기준날짜 + 1일 -1일 단위로 계산이 진행됨
...
-- 테이블에 원래 없는 값인데, 리터럴 사용시 존재하는 것처럼 사용 가능

-- (필수) DB의 리터럴 표기법은 ' ' 홑따옴표
-- " " 쌍따옴표는 특수문자, 대소문자, 기호 등을 구분하여 나타낼 때 사용하는 표기법
-- 쌍따옴표 안에 작성되는 것들은 하나의 단어이다.
...
-- 1. FROM 절 : FROM 테이블명
-- 2. WHERE 절 : WHERE 컬럼명 연산자 값(조건절)
-- 4. ORDER BY 절 : ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ ASC | DESC [NULLS FIRST | LAST ] 
--											   (오름차순 | 내림차순)
-- EMPLOYEE 테이블에서 (급여가 300만원 초과인 사원) 의 (조건 WHERE) 
-- 사번 이름 급여 부서코드를 조회
...
-- EMPLOYEE 테이블에서 입사일이 1990-01-01 ~ 1999-12-31 사이 인 직원의
-- 이름, 입사일 조회
...
-- EMPLOYEE 테이블에서 EMAIL의 _ 앞 글자가 세 글자인 사원을 조회
 ...
 */
 ...
 -- EMPLOYEE 테이블에서
 -- 부서 코드가 D1,D6,D9인 사원의
 -- 사번, 이름, 부서코드 조회
 

-- IN
 SELECT EMP_ID, EMPTY_NAME, DEPT_CODE
 FROM EMPLOYEE
...
*/
 ...
 OR DEPT_CODE IS NOT NULL; -- 부서코드 없는 2명 포함해서 14명
...
-- ORDER BY 절은 SQL 문장으로 조회된 데이터들을 다양한 목적에 맞게
-- 특정 칼럼을 기준으로 정렬하여 출력 하는데 사용한다
...
FROM EMPLOYEE
ORDER BY SALARY; -- 오름차순 이지만, ASC가 기본값이기 때문에 안써줘도 됨
...
-- D1부서에서 가장 많은 ~ 가장 적은 사람까지 내림차순 정렬
-- D2부서에서 ...



-- 정렬중첩 : 대분류 정렬 이후 소분류 정렬 --
